          ...
          pkgs.writeShellScript "protonvpn-port-forwarding" ''
            ...
            while true; do
              ...
              if [[ -n "$PORT" && "$PORT" != "$OLD_PORT" ]]; then
                ...
                TR_AUTH="${config.services.transmission.settings.rpc-username}:$(cat ${config.sops.secrets.transmission-password.path})" transmission-remote localhost --port "$PORT" --authenv --no-portmap
              fi
